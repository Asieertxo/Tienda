...
function Example(){
  return(
    <section className='slider'>
			<h1 className='slider-title'>Carousel con infinite-react-carousel</h1>
			<Slider className='slider-content'>
				{list.map((image) => (
					<div key={image.ObjectID} className='slider-content__item'>
						<img src={image.img} alt={image.name} />
            <img src={image.img} alt={image.name} />
						<p className='slider-content__description'>{image.name}</p>
					</div>
				))}
			</Slider>
		</section>
